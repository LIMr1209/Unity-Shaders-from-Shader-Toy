    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float noise2(float2 x)
            {
                ...
                float4 h = hash4(float4(n, n, n, n) + float4(NC0.xy, NC1.xy));
            ...
            float noise3(float3 x)
            {
                ...
                float4 s1 = lerp(hash4(float4(n, n, n, n) + NC0), hash4(float4(n, n, n, n) + NC1), f.xxxx);
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                float4 refcolor = float4(0, 0, 0, 0);
                iteratorc = iterations - layers;

                float2 addrot = float2(0, 0);
                ...
                {
                    float2 p2 = p - float2(0.25, 0.25) + float2(0.1 * float(i), 0.1 * float(i));
                    ...
                    if (noise3(seedn) > 0.2 && i < int(layers))
                    {
                        ...
                        mr = float3x3(float3(coss.x, 0.0, sins.x), float3(0.0, 1.0, 0.0), float3(-sins.x, 0.0, coss.x));
                        mr = float3x3(float3(1.0, 0.0, 0.0), float3(0.0, coss.y, sins.y),
                                      float3(0.0, -sins.y, coss.y)) * mr;
                        mr = float3x3(float3(coss.z, sins.z, 0.0), float3(-sins.z, coss.z, 0.0),
                                      float3(0.0, 0.0, 1.0)) * mr;

                        // light = normalize(float3(1.0, 0.0, 1.0)) * mr;
                        light = mul(normalize(float3(1.0, 0.0, 1.0)), mr);
                ...
                float3 cr = lerp(float3(0, 0, 0), float3(0.0, 0.0, 0.4), (-0.55 + p.y) * 2.0);
                fragColor.xyz += lerp((cr.xyz - fragColor.xyz) * 0.1, float3(0.2, 0.5, 1.0),
                                      clamp((-p.y + 1.0) * 0.5, 0.0, 1.0));

                fragColor = min(float4(1, 1, 1, 1), fragColor);
                fragColor.a = 1.0;
                return fragColor;
            }
            ENDCG
